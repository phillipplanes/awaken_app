...
// --- MAX98357A I2S pins on ESP32-WROOM-32E ---
...
void connectToWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        wifiConnected = false;
        Serial.println("WiFi failed; alarm clock time trigger disabled");
    }
}
...
bool startFilePlayback(const char* filePath, bool loopPlayback) {
    stopFilePlayback();
...
void triggerAlarm() {
    ...
    if (alarmSoundEnabled && uploadedVoiceReady) {
        startFilePlayback(uploadedVoiceFile, true);
    }
}
...
class MyServerCallbacks: public BLEServerCallbacks {
    void onConnect(BLEServer *) override {
        ...
        if (!a2dpConnected) {
            ...
            a2dpSink.reconnect();
            Serial.println("A2DP reconnect requested after BLE connect");
        }
    }
...
class SpeakerControlCallback: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pChar) override {
        ...
        if (cmd == SPEAKER_CMD_ALARM_SOUND_ON) {
            ...
            if (alarmFiring && uploadedVoiceReady) {
                startFilePlayback(uploadedVoiceFile, true);
            }
            return;
        }
        ...
        if (cmd == SPEAKER_CMD_PLAY_UPLOADED) {
            if (uploadedVoiceReady) {
                startFilePlayback(uploadedVoiceFile, false);
            } else {
...
void setupBle() {
    ...
    // Keep compatibility with app expectations.
    service->createCharacteristic(INTENSITY_CHAR_UUID, BLECharacteristic::PROPERTY_WRITE);
    service->createCharacteristic(EFFECT_CHAR_UUID, BLECharacteristic::PROPERTY_WRITE);
    service->createCharacteristic(WAKE_EFFECT_CHAR_UUID, BLECharacteristic::PROPERTY_WRITE);
...
void setupA2dp() {
    ...
    a2dpSink.set_mono_downmix(false);
    ...
    a2dpSink.start(a2dpName, false); // no auto reconnect on boot
    a2dpSink.set_connectable(true);
    a2dpSink.set_discoverability(ESP_BT_GENERAL_DISCOVERABLE);
...
void setup() {
    ...
    delay(250);
    Serial.println();
    Serial.println("Awaken hybrid firmware: BLE + Bluetooth Classic A2DP + Alarm");
    initDeviceNames();
